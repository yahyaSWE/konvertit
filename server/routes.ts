...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use(
    session({
      secret: process.env.SESSION_SECRET || 'learnsmart-lms-secret',
      resave: false,
  ...
  const isTeacher = (req: Request, res: Response, next: any) => {
    if (
      req.isAuthenticated() &&
      ((req.user as any).role === UserRole.TEACHER || (req.user as any).role === UserRole.ADMIN)
    ) {
      return next();
    }
  ...
  async function updateCourseProgress(userId: number, lesson: any) {
    try {
      ...
      if (progressPercentage === 100 && !enrollment.completed) {
        ...
        const firstCourseAchievement = (await storage.getAllAchievements()).find(
          (a) => a.title === 'First Course Completed',
        );

        if (firstCourseAchievement) {
          const userHasAchievement = await storage.getUserAchievement(userId, firstCourseAchievement.id);
